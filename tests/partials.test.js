...
describe('partials', function () {
	...
	it('partials with duplicate parameters', function () {
		expectTemplate('Dudes: {{>dude dudes foo bar=baz}}')
			.withPartial('dude', '')
			.toThrow(/^Unsupported number of partial arguments: 2/);
	});
	...
	it('registering undefined partial throws an exception', function () {
		expectTemplate('one')
			.withPartial('undefined_test', undefined)
	...
	describe('compat mode', function () {
		it('partials can access parents', function () {
			expectTemplate('Dudes: {{#dudes}}{{> dude}}{{/dudes}}')
				...
				.toCompileTo('Dudes: Yehuda (http://yehuda) yes Alan (http://alan) yes ');
		});
